...
export default function Tree({ results, selectedFile, onFileSelection, onToggle }: Props) {
  return (
    <FileTreeContainer>
      <AutoResizer>
        {({ height, width }: any) => {
          return (
            <List height={height - 90} itemCount={results.length} itemSize={20} width={width}>
              {({ index, style }: any) => {
                return (
                  <FileItem
                    style={style}
